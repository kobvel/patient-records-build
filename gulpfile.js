...
var config = {
  ...
  winIco: './icons/patient.ico',
  macIcns: './icons/patient.icns',
  macPlist: {
    mac_bundle_id: 'Patient'
  }
};
...
availablePlatforms.forEach(function(platform) {
  gulp.task('clean:nw:' + platform, function() {
    return gulp.src('build/patient/' + platform + '/*', {
        read: false
      })
      .pipe(rimraf({
        force: true
...
gulp.task('box64', ['nw:win64'], function() {
  var res = ['patient.evp', 'patient.exe'].map(function(i) {
    return './release/win64/' + i;
  }).concat(['patient-records.exe',
    'nw.pak', 'icudtl.dat', 'ffmpegsumo.dll', 'libEGL.dll',
    'libGLESv2.dll'
  ].map(function(i) {
    return './build/patient-records/win64/' + i;
  }));
...
gulp.task('box32', ['nw:win32'], function() {
  var res = ['patient.evp', 'patient.exe'].map(function(i) {
    ...
    return './build/patient-records/win32/' + i;
  }));
